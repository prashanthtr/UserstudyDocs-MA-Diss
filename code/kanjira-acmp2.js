...
var kanjira_patterns = [["ta","tum","tumki","."],["ta tum", "tum ta","ta te","ta.",".ta","te.",".te","tum.",".tum"], ["tumki. tumki. ta thum thum ta"],["ta thum thum ta"],["ta ta thum thum ta thum thum ta"]];
...
//cur_acc = kanjira_patterns[Math.floor( 2.6 + 2*Math.random())];
	var kanjira_pattern;// = "[" + cur_acc + "," + 2 + "]";
...
return sh.track( sh.repeat( times , sh.track(accomp.map(function(s,index){
	...
	if(s == "."){
		...
		return sh.track([sounds[s].trigger(acmpVolume), sh.delay(1.0)]);
	}
